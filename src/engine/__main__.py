...
from engine.api import AllBarcodeClassifiers, Engine, AllActions, FullGraphState, SingleBarcodeClassifier, SingleNodeState

from panda3d.core import Filename, PNMImage, NodePath, GeomVertexReader, Point3, Vec3 # Added Point3, Vec3
...
def take_screenshot(app_instance: ShowBase, filepath: Optional[str] = None, cam_pos: Optional[Tuple[float, float, float]] = None, cam_hpr: Optional[Tuple[float, float, float]] = None, return_data: bool = False) -> Optional[PNMImage | bool]:
    ...
    if not win or not cam or not graphics_engine:
        print("Error: Panda3D ShowBase components (win, cam, graphicsEngine) not available.")
        return None
    ...
    if cam_pos is not None or cam_hpr is not None:
        ...
        if cam_pos: cam.set_pos(*cam_pos) # Unpack tuple
        if cam_hpr: cam.set_hpr(*cam_hpr) # Unpack tuple
    ...
    if return_data:
        ...
        if graphics_engine.doYield():
            ...
            if success and not img.is_null(): # Check if image data is valid
                ...
                print(f"Warning: Failed to capture screenshot data using getScreenshot(). Success={success}, Null={img.is_null()}")
                result = None
        else:
             print("Warning: graphics_engine.doYield() failed, cannot guarantee screenshot data.")
             result = None
...
class App(ShowBase):
    ...
    def __init__(self):
        global base # Allow modification of the global 'base' reference
        if len(sys.argv) < 2:
        ...
        super().__init__()
        base = self # Set the global reference to this instance
        ...
        # self.accept('y', self.toggle_ambient_light) # If you want ambient toggle
        self.accept('a', self._generate_new_nodes, [5]) # Bind 'a' to generate 5 nodes internally
        ...
        # Example binding for custom view screenshot (replace with actual desired coords)
        self.accept("f2", self.take_custom_view_screenshot, [(0, -20, 5), (0, -15, 0)]) # Example pos/hpr
        ...
        if min_pt == max_pt: # Check if bounds calculation failed or model is a point
             print("Warning: Model bounds are zero or failed to compute. Using default camera position and radius.")
             ...
             if not bounds.is_empty():
                 ...
                 if radius < 0.1: radius = 0.5 # Clamp small radius
             else:
                 print("Error: Both tight bounds and regular bounds failed. Using origin.")
        else:
             # Calculate radius based on the tight bounds
             radius = (max_pt - center).length()
             if radius < 0.1:
                  print(f"Warning: Model radius very small ({radius:.3f}). Clamping to 0.5 for camera distance.")
                  radius = 0.5
             print(f"Model Tight Bounds Center: {center.getX():.2f}, {center.getY():.2f}, {center.getZ():.2f}")
        ...
        near_plane = max(0.01, radius / 100.0) # Avoid zero near plane, use smaller value relative to radius
        far_plane = max(100.0, default_distance + radius * 5.0) # Ensure far plane covers model + distance
        ...
        self.ambient_light_node.set_color((0.3, 0.3, 0.3, 1)) # Slightly brighter ambient
        ...
        # --- Animation ---
        char_node = self.model_root.find('**/+Character') # Find any node with Character component
        if char_node:
            self.anims = p3d.AnimControlCollection()
            p3d.autoBind(self.model_root.node(), self.anims, ~0) # Bind animations under model_root
    ...
    def take_custom_view_screenshot(self, pos: tuple[float, float, float], hpr: tuple[float, float, float]):
        ...
        print(f"Taking screenshot from custom view: Pos={pos}, HPR={hpr} to {filepath}")
         # Pass self (the App instance) to the screenshot function
        success = take_screenshot(self, filepath=filepath, cam_pos=pos, cam_hpr=hpr, return_data=False)
        if success:
            print("Screenshot saved.")
        else:
            print("Screenshot failed.")
    ...
    def _generate_new_nodes(self, sample_size: int = 1) -> None:
        ...
        min_pt, max_pt = Point3(), Point3()
        self.model_root.calc_tight_bounds(min_pt, max_pt, self.render) # Calculate in world space

        if min_pt == max_pt:
             print("Warning: Could not get tight bounds for node generation. Using potentially less accurate bounds.")
        ...
        print(f"Using Bounds for Generation: Min({min_x:.2f}, {min_y:.2f}, {min_z:.2f}), Max({max_x:.2f}, {max_y:.2f}, {max_z:.2f})")
        print(f"Center: ({center.x:.2f}, {center.y:.2f}, {center.z:.2f}), Radius: {radius:.2f}")
        ...
        dist_min = radius * (dist_factor * 0.8) + max_span * 0.5 # Closer for smaller span
        dist_max = radius * (dist_factor * 1.2) + max_span * 1.0 # Further for larger span
        ...
        z_offset_max = span_z * 1.0 + radius * 0.5 # Look from higher up, relative to radius
        ...
        for i in range(sample_size):
            ...
            phi_angle_deg = random.uniform(15, 75) # Angle from Z+ axis (15=high, 75=lower angle)
            ...
            cam_z = center.z + max(z_offset_min, offset_z) # Ensure camera is reasonably high
            ...
            print(f"Node {count}: Pos({pos[0]:.2f}, {pos[1]:.2f}, {pos[2]:.2f}), HPR({rot[0]:.1f}, {rot[1]:.1f}, {rot[2]:.1f}) -> Target({target_x:.2f}, {target_y:.2f}, {target_z:.2f})")

            image_data = take_screenshot(self, cam_pos=pos, cam_hpr=rot, return_data=True)

            if image_data is None or image_data.is_null():
                 print(f"Warning: Failed to get valid screenshot data for node {current_node_id}. Skipping node.")
            ...
            for node_id in position_of_other_nodes.keys(): # Iterate through IDs that existed *before* this loop iteration
                 # Check if the node still exists (might be relevant in complex scenarios)
                if node_id in full_graph:
                    full_graph[node_id].position_of_other_nodes[current_node_id] = pos
                    full_graph[node_id].rotation_of_other_nodes[current_node_id] = rot

            count += 1 # Increment global count *after* successfully adding node

        print(f"--- Camera Node Generation Complete ({len(newly_added_nodes)} nodes added) ---")
...
def step(actions: AllActions) -> tuple[FullGraphState, AllBarcodeClassifiers]:
    ...
    for (i, sa) in all_actions.items():
        delta_pos = cast(tuple[float, float, float], tuple(x + y for x, y in zip(delta_pos, sa.delta_pos)))
        delta_rot = cast(tuple[float, float, float], tuple(x + y for x, y in zip(delta_rot, sa.delta_rot)))
        

    current_classifiers: AllBarcodeClassifiers = {}

    for (i, sns) in full_graph.items():
        sns.position = cast(tuple[float, float, float], tuple(x + y for x, y in zip(sns.position, delta_pos)))
        sns.rotation = cast(tuple[float, float, float], tuple(x + y for x, y in zip(sns.rotation, delta_rot)))
        current_classifiers[i] = SingleBarcodeClassifier((sns.depth_map.width, sns.depth_map.height), sns.depth_map, i)
